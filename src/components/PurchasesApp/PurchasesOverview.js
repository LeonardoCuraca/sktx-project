...
export default function PurchasesOverview() {
    ...
    const getCustomers = React.useCallback(async () => {
        PurchasesService.getAllPaginated({
            "page": actualPage,
            "size": 10
        })
        .then(data => {
                    ...
                    <tbody>
                        {
                            purchases.map(purchase => {
                                return <tr key={purchase.productPurchaseId} style={{cursor: 'pointer'}} onClick={() => history.push(`${url}/${purchase.productPurchaseId}/detail`)}>
                                    <td className="center aligned">{purchase.productPurchaseId}</td>
                                    <td>{moment(purchase.createdAt).format('L, hh:mm:ss')}</td>
                                    <td>{purchase.Provider.providerNames}</td>
                                    <td>{purchase.User.userNames}</td>
                                    <td>{purchase.ProductPurchaseDetails.length}</td>
                                </tr>
                            })
                        }
                    </tbody>
